...
//		}
//		//将收取到的数据拷贝到消息缓冲区
//		memcpy(pClient->rcvMsg() + pClient->getLastPos(), _szRecv, nLen);
//		//消息缓冲区的数据尾部位置后移
...
//		{
//			//这时就可以知道当前消息的长度
//			DataHeader* header = (DataHeader*)pClient->rcvMsg();
//			//判断消息缓冲区的数据长度大于消息长度
//			if (pClient->getLastPos() >= header->dataLen)
//			{
//				//消息缓冲区剩余未处理数据的长度
//				int nSize = pClient->getLastPos() - header->dataLen;
//				//处理网络消息
//				OnNetMsg(pClient->sockfd(), header);
//				//将消息缓冲区剩余未处理数据前移
//				memcpy(pClient->rcvMsg(), pClient->rcvMsg() + header->dataLen, nSize);
//				//消息缓冲区的数据尾部位置前移
...
//			//printf("收到客户端<Socket=%d>请求：CMD_LOGIN,数据长度：%d,userName=%s PassWord=%s\n", cSock, login->dataLen, login->userName, login->PassWord);
//			//忽略判断用户密码是否正确的过程
//			//LoginResult ret;
//			//SendData(cSock, &ret);
//		}
...
//			//printf("收到客户端<Socket=%d>请求：CMD_LOGOUT,数据长度：%d,userName=%s \n", cSock, logout->dataLen, logout->userName);
//			//忽略判断用户密码是否正确的过程
//			//LogoutResult ret;
//			//SendData(cSock, &ret);
//		}
...
public:
	...
	int RecvData(ClientSocket *pCSock) {
		...
		int rcvBuffLen = recv(pCSock->sockfd(), _rcvBuff, RECV_BUFF_SIZE, 0);
		if (rcvBuffLen <= 0)
		{
...
//	void Start()
//	{
//		for (int n = 0; n < SERVER_THREAD_COUNT; n++)
//		{
